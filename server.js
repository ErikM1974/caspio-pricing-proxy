...
app.post('/api/quote_items', express.json(), async (req, res) => {
    try {
        const {
            ...
            SizeBreakdown, PricingTier, ImageURL, ItemID
            // PK_ID is auto-generated by Caspio
        ...
        // Get the next available ItemID if not provided
        let nextItemID = ItemID || 100; // Default starting value
        if (!ItemID) {
            try {
                // Try to get the highest existing ItemID
                const resource = '/tables/Quote_Items/records';
                const params = {
                    'q.select': 'ItemID',
                    'q.orderby': 'ItemID DESC',
                    'q.limit': 1
                };
                
                const result = await fetchAllCaspioPages(resource, params);
                if (result && result.length > 0 && result[0].ItemID) {
                    nextItemID = parseInt(result[0].ItemID) + 1;
                }
            } catch (error) {
                console.warn("Could not determine next ItemID, using default:", error.message);
            }
        }
        ...
        const recordData = {
            ...
            ItemID: nextItemID // Add the required ItemID field
        };
