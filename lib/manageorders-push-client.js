...
async function transformOrder(orderData) {
  ...
  const manageOrdersOrder = {
    ...
    // APISource: routes orders to a specific ShopWorks ManageOrders integration.
    // ShopWorks's tooltip: "Leave empty to pull all orders" — meaning each
    // integration with a blank APISource pulls every order. To avoid
    // duplicates, an integration with a specific APISource value only pulls
    // orders that include that exact value here. Order Form sends
    // "OrderForm"; legacy callers (3-Day Tees, embroidery push) omit this
    // field, so they keep flowing through the existing blank-APISource
    // integration unchanged.
    ...(orderData.apiSource ? { APISource: orderData.apiSource } : {}),
