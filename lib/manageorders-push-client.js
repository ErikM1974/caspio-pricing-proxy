...
async function transformOrder(orderData) {
  ...
  const manageOrdersOrder = {
    ...
    cur_Shipping: orderData.shippingAmount || 0,
  };
  ...
  manageOrdersOrder.Customer = {
    ...
    // Custom Fields (10 fields - for future extensibility)
    CustomField01: orderData.customer?.customFields?.CustomField01 || '',
    CustomField02: orderData.customer?.customFields?.CustomField02 || '',
    CustomField03: orderData.customer?.customFields?.CustomField03 || '',
    CustomField04: orderData.customer?.customFields?.CustomField04 || '',
    CustomField05: orderData.customer?.customFields?.CustomField05 || '',
    CustomField06: orderData.customer?.customFields?.CustomField06 || '',
    CustomDateField01: orderData.customer?.customDateFields?.CustomDateField01 || '',
    CustomDateField02: orderData.customer?.customDateFields?.CustomDateField02 || '',
    CustomDateField03: orderData.customer?.customDateFields?.CustomDateField03 || '',
    CustomDateField04: orderData.customer?.customDateFields?.CustomDateField04 || ''
  };
...
function transformLineItems(lineItems) {
  return lineItems.map((item, index) => {
    ...
    const lineItem = {
      ...
      Qty: item.quantity,
      Price: item.price || 0,
      id_ProductClass: ONSITE_DEFAULTS.id_ProductClass,
...
async function uploadFilesToCaspio(files) {
  ...
  for (const file of files) {
    try {
      ...
      // Call existing /api/files/upload endpoint
      console.log(`[File Upload] Uploading ${file.fileName}...`);
      const response = await axios.post(
        `${PROXY_BASE_URL}/api/files/upload`,
        {
          fileName: file.fileName,
          fileData: file.fileData,
          description: file.description || `File for order`
        },
        {
      ...
      if (response.data.success) {
        uploadedFiles.push({
          ...
          caspioUrl: `${PROXY_BASE_URL}/api/files/${response.data.externalKey}`,
        ...
        console.log(`[File Upload] Uploaded: ${file.fileName} → ${response.data.externalKey}`);
      }
    } catch (error) {
      console.error(`[File Upload] Failed to upload ${file.fileName}:`, error.message);
...
function transformAttachments(uploadedFiles) {
  ...
  return uploadedFiles.map(file => ({
    ...
    Link: 0  // 0 = media file hosted on Caspio, 1 = external URL reference
  }));
}
