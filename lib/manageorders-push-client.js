...
const { swImageUrl } = require('./sw-image-url');
...
async function transformOrder(orderData) {
  ...
  //
  // ExtCustomerID priority (Erik 2026-05-21 audit C2):
  //   1. orderData.customer.companyId — the CRM Customer ID from
  //      CompanyContactsMerge2026 / ShopWorks. STABLE per customer across
  //      all their orders. ExtCustomerID becomes "NWCA-CUST-107" for
  //      Star Sportswear regardless of which OF# it's pushed under.
  //   2. orderData.orderNumber — per-order fallback when companyId is empty
  //      (brand-new typed-name customer the rep didn't pick from autocomplete).
  //      Generates "NWCA-CUST-OF-0048" — unique but ephemeral; SW's customer
  //      record then has only this single order attached.
  //   id_Customer (numeric ShopWorks ID) is still the primary dedup key
  //   for SW's matcher; ExtCustomerID is the fallback for ones it can't
  //   resolve via id_Customer.
  const stableCustomerId = orderData.customer && orderData.customer.companyId
    ? String(orderData.customer.companyId).trim()
    : '';
  const customerExtId = stableCustomerId
    ? `${ONSITE_DEFAULTS.ExtCustomerPref}-CUST-${stableCustomerId}`
    : `${ONSITE_DEFAULTS.ExtCustomerPref}-CUST-${orderData.orderNumber}`;

  const manageOrdersOrder = {
    ExtOrderID: extOrderID,
    ExtSource: ONSITE_DEFAULTS.ExtSource,
    ExtCustomerID: customerExtId,
    ExtCustomerPref: ONSITE_DEFAULTS.ExtCustomerPref,
    // APISource routes to the OnSite "Manage Orders" integration, which imports ONLY
    // orders whose APISource exactly equals its filter value ("ManageOrders"). Per Erik
    // 2026-06-04, we stamp "ManageOrders" on EVERYTHING we push (EMB/SCP/DTF/3-Day Tees/
    // Order Form) so one integration imports them all. This intentionally OVERRIDES any
    // orderData.apiSource (e.g. the Order Form's former "OrderForm") — the value is now
    // uniform across every push path. A non-matching/blank value = silently skipped import.
    APISource: ONSITE_DEFAULTS.APISource || 'ManageOrders',
    ...
    // OnSite configuration defaults — caller can override per-order.
    // Order Form sends idCustomer (real ShopWorks customer ID when the rep
    // picked a known company from autocomplete), so honor it when present
    // and fall back to the webstore catch-all (2791) for callers that
    // don't supply one (3-Day Tees, embroidery push).
    id_Customer: orderData.idCustomer || ONSITE_DEFAULTS.id_Customer,
    id_OrderType: orderData.idOrderType || ONSITE_DEFAULTS.id_OrderType,
    id_CompanyLocation: orderData.idCompanyLocation || ONSITE_DEFAULTS.id_CompanyLocation,
    id_EmpCreatedBy: orderData.idEmpCreatedBy || ONSITE_DEFAULTS.id_EmpCreatedBy,
    ...
    CustomerPurchaseOrder: orderData.customerPurchaseOrder || orderData.purchaseOrderNumber || '',
  ...
  //
  // SKIP the Customer block when the caller resolved a real ShopWorks
  // customer ID (idCustomer differs from the default 2791 catch-all).
  // ShopWorks already has the canonical billing record for that customer —
  // sending the form-typed billing data on every order would risk
  // overwriting their record with a rep's typo. The Order Form's case:
  // when info.companyId mapped to a real id_Customer, we don't need to
  // re-state the customer's billing on the order header.
  //
  // Webstore catch-all flows (3-Day Tees, embroidery push) keep the
  // current behavior — they always land on customer 2791, so the
  // Customer block carries the per-order contact info as today.
  const isRealCustomer = orderData.idCustomer && orderData.idCustomer !== ONSITE_DEFAULTS.id_Customer;
  if (!isRealCustomer) {
  ...
  // Customer info is now included in frontend notes - no need to add here
  ...
  if (orderData.attachments && Array.isArray(orderData.attachments) && orderData.attachments.length > 0) {
    ...
    const transformedAttachments = orderData.attachments.map(attachment => ({
      MediaURL: swImageUrl(attachment.mediaUrl || ''), // P2: ≤2MB JPEG variant for our own file URLs
    ...
    if (artworkFiles.length > 0) {
      ...
      manageOrdersOrder.Designs.push({
        ...
        Locations: artworkFiles.map(file => ({
          ...
          ImageURL: swImageUrl(file.caspioUrl), // P2: ≤2MB JPEG variant
...
function transformLineItems(lineItems) {
  return lineItems.map((item, index) => {
    ...
    // Detect fee/service lines (DD, GRT-50, 3D-EMB, AL-CAP, etc.) so we can
    // route them through the right Size translation path. Service codes have
    // no garment size — historically we sent Size: '' and qty in Size01, but
    // ShopWorks's Size Translation Table treats blank Size as "fall through to
    // last column" (column 6 / Other XXXL). The configured service parts only
    // accept qty in column 1 (Adult/S), so a blank Size silently misroutes.
    // Fix: stamp Size: 'S' so the existing S → Adult/column 1 rule handles
    // every service code without per-code translation entries. See
    // memory/MANAGEORDERS_COMPLETE_REFERENCE.md §Order Form / Size:"S" rule.
    const isFeeCode = isKnownFeeCode(item.partNumber);

    // Translate size using SIZE_MAPPING — except fee lines, which always 'S'.
    ...
    // Color: prefer the abbreviated mainframe code (CATALOG_COLOR) over the
    // friendly display name. SanMar's PromoStandards API returns CATALOG_COLOR
    // values verbatim (e.g. "Bk/Bk/LtGy"), and ShopWorks must forward that
    // exact string on the SanMar PO. If we send the friendly name
    // ("Black/ Black/ Light Grey"), ShopWorks has to translate via its own
    // product table — and any miss in that translation produces a bad PO and
    // a SanMar rejection. Verified for style 112 (2026-05-02): all 112
    // CATALOG_COLOR values match SanMar's live colorName 1:1. Falls back to
    // item.color for clients that don't send catalogColor (3-Day Tees,
    // embroidery push) — backward compatible.
    const lineItem = {
    ...
    // Fee/service lines: put qty in Size01 (column 1) and zero the rest, so
    // ShopWorks's quantity column matches the part's Size Restriction (S only).
    if (isFeeCode) {
      ...
      lineItem.Size05 = 0;
      lineItem.Size06 = 0;
    } else if (item.useSizeColumn) {
      // Legacy callers that explicitly opt into size-column mode (no fee match)
      lineItem.Size01 = item.quantity;
    }
...
function transformDesigns(designs) {
  return designs.map((design, index) => {
    // id_Design: ONLY emit when caller resolved a real ShopWorks design ID.
    // Sending `id_Design: 0` makes ShopWorks attach line items to a sentinel
    // orphan-design row instead of creating a new design from DesignName +
    // ExtDesignID. Per memory/MANAGEORDERS_COMPLETE_REFERENCE.md the field is
    // optional — omit when unknown so ShopWorks creates a fresh design.
    ...
    const transformedDesign = {
      ...
      ...(resolvedDesignId ? { id_Design: resolvedDesignId } : {}),
...
async function verifyOrder(extOrderId) {
  try {
    ...
    // Look back 7 days. Pushes typically appear in /order-pull within minutes,
    // but a wider window catches edge cases (clock skew, late uploads, orders
    // pushed via share-link drafts the rep finalized hours later).
    ...
    console.log(`[ManageOrders PUSH] Verifying order ${extOrderId} (window ${dateFrom} → ${dateTo})...`);

    // ⚠ DO NOT filter by api_source here. The /order-pull `api_source` param
    // filters by the order's `APISource` FIELD (e.g. "OrderForm"), NOT the
    // `ExtSource` field. Passing ExtSource ("NWCA") here would exclude every
    // OrderForm order (APISource="OrderForm") AND every legacy webstore
    // order (APISource blank). Better: pull everything in the date window
    // and scan in-memory by ExtOrderID. Today's volume is small enough.
    ...
    //
    // ⚠ RESPONSE SHAPE (verified 2026-05-20):
    // The official Swagger spec shows `result: [{ order_json: {...} }]` but the
    // ACTUAL API returns order fields flat at the top level of each result entry:
    //   { result: [{ ExtOrderID, ExtSource, date_OrderPlaced, ... }] }
    // Previously this code read `order.order_json.ExtOrderID` which was ALWAYS
    // undefined — meaning verifyOrder silently returned "not found" for every
    // order ever pushed. Defensive: try both shapes so we still work if MO
    // ever switches to the documented Swagger shape.
    const orders = response.data.result || [];
    const foundOrder = orders.find(order => {
      const o = order.order_json || order; // fall back to flat if wrapper missing
      return o.ExtOrderID === extOrderId;
    });

    if (foundOrder) {
      const orderData = foundOrder.order_json || foundOrder;
      console.log(`[ManageOrders PUSH] Order ${extOrderId} verified successfully`);
      return {
        success: true,
        found: true,
        extOrderId,
        uploadedAt: orderData.date_OrderPlaced || 'unknown',
        orderData
      };
    } else {
