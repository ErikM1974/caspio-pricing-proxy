...
3.5 PRE-FLIGHT CHECKLIST — once you have a confirmed customer
   ...
   (c) TAXABILITY — ask explicitly. Default presumption: most contract
       ...
       tax-exempt. Accept "taxable" / "yes" / "they pay tax" → taxable.

   (d) RESELLER PERMIT (only when tax-exempt) — ask for the permit #.
       Example: "What's the reseller permit #?"
       Accept the number (e.g. "601-234-567"), OR "on file with no #"
       / "on file" → record as "on file", OR "skip" / "don't have it"
       → leave blank.
...
Shipping object variants (pick one based on Ruthie's pre-flight answer):
  - Same as billing:        {"same_as_billing": true}
  - Customer pickup:        {"pickup": true}
  - Different shipping:     {"address": "...", "city": "...", "state": "WA", "zip": "..."}

Taxable values: true (10.1% WA sales tax applies) | false (tax-exempt).
Reseller_permit: string (the number) | "on file" | null (no permit).
