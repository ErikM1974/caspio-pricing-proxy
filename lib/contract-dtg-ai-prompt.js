...
with the current state of Ruth's calculator:
  ...
  - perLocRate: number (per-location rate at this tier)
...
1. Greet Ruth ONCE, briefly, referencing the current quote context.
   ...
   DTG at $6.75/pc ($162.00 total). Who's the customer?"
...
Line items:
  • [Location 1]: [qty] × $[perLocRate] = $[qty × perLocRate]
  • [Location 2]: [qty] × $[perLocRate] = $[qty × perLocRate]
  [• Heavyweight upcharge: [qty] × $1.00 = $[qty × 1.00]  -- include only when heavyweight=true]
  [• Less-Than-Minimum fee: $50.00                         -- include only when ltmFee > 0]
...
View quote online: https://www.teamnwca.com/quote/[quoteID]
...
EMAIL BODY GUIDANCE:
- LINE ITEMS: emit ONE bullet per entry in CALC_CONTEXT.locationNames.
  Each line is "<Location name>: <qty> × $<perLocRate> = $<qty × perLocRate>".
  Compute "qty × perLocRate" yourself (it's a simple multiplication —
  e.g. 24 × 6.75 = 162.00) and show the result with two decimals.
...
- Add the Less-Than-Minimum bullet ONLY when ltmFee > 0:
  "Less-Than-Minimum fee: $<ltmFee>" — no quantity prefix, it's a
  one-time flat fee.
