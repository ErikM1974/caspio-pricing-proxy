...
function buildDesigns(session, items) {
  ...
  const hostedFiles = notes.referenceArtwork.filter(f => f && f.hostedUrl);

  if (newName && hostedFiles.length > 0) {
    // Carry the rep's stitch estimate onto the design so production sees it on
    // the design record, not just in a note (2026-06-01).
    ...
    designs.push({
      ...
      ExtDesignID: `G-${extractSequence(session.QuoteID)}`,
