...
// Display names for Non_SanMar_Products.VendorCode. Mirrors NON_SANMAR_VENDORS in the
// app's shared_components/js/quote-builder-utils.js — a LABEL vocabulary, never a price.
// Unknown codes fall through verbatim so one-off vendors still read sensibly on the order.
const NON_SANMAR_VENDOR_NAMES = {
  SSA: 'S&S Activewear',
  ALP: 'Alphabroder',
  CARH: 'Carhartt',
  RICH: 'Richardson',
  JDS: 'JDS Industries',
  AUG: 'Augusta / Holloway',
};
...
function buildProductLines(item, garmentDesignId, capDesignId) {
  ...
  // ALLOWLIST of the metadata keys the frontend actually writes into SizeBreakdown.
  // Everything NOT listed here is treated as a SIZE and becomes a real LinesOE line, so
  // any new metadata key added on the write side MUST be added here too — otherwise it
  // ships as a phantom line with Size/Qty set to the key and a real Price on it.
  // ('heavyweight' is written by the customer-supplied save path; it only survives today
  // because those items route to buildServiceLine, not here.)
  const sizeEntries = Object.entries(sizeBreakdown).filter(
    ([key]) => !['type', 'serviceType', 'logoPosition', 'stitchCount', 'heavyweight'].includes(key)
  );

  // Vendor (non-SanMar) provenance, written by the builder into LogoSpecs.ns. Whoever
  // raises the PO needs to see that this blank is NOT a SanMar part. Tolerant parse:
  // LogoSpecs is free-form and most items have no `ns` block.
  //
  // ⚠️ Fires only for CATALOGUED vendor products. A MANUAL item (typed straight onto the
  // line) carries `ns.v === ''` because it has no VendorCode — the vendor picker went with
  // the Add-Product modal on 2026-08-15. That is a decision, not a gap: the vendor rides in
  // the rep's DESCRIPTION, which is already sent as `Description` below. Do not "fix" the
  // empty note by inventing a vendor here. (Confirmed on TEST push EMB-TEST-2026-315.)
  let vendorNote = '';
  let vendorDescription = '';
  try {
    const specs = JSON.parse(item.LogoSpecs || '{}');
    const vendorCode = specs && specs.ns && specs.ns.v;
    if (vendorCode) {
      const name = NON_SANMAR_VENDOR_NAMES[String(vendorCode).toUpperCase()] || vendorCode;
      vendorNote = `VENDOR: ${name} — not a SanMar part`;
      vendorDescription = `${item.ProductName || ''} (${name})`;
    }
  } catch {
    // Malformed LogoSpecs must never block a push — just ship without the vendor note.
  }
  ...
  if (sizeEntries.length > 0) {
    ...
    for (const [size, qty] of sizeEntries) {
      ...
      lines.push({
        ...
        NameFirst: '',
        NameLast: '',
        LineItemNotes: vendorNote,
        WorkOrderNotes: '',
        DesignIDBlock: '',
        // PartNumber stays the real style; DisplayAsPartNumber is deliberately left
        // empty and held in reserve as the fallback if OnSite rejects unknown parts.
        DisplayAsPartNumber: '',
        DisplayAsDescription: vendorDescription,
      });
    }
  } else {
...
function buildNotes(session, items, skippedFeeNotes = [], taxInfo = {}) {
  ...
  // Customer-supplied goods: what is arriving, and how many of each size. Receiving is
  // the department that counts the customer's boxes in, and until now nothing on the
  // order told them what to expect — the line just read "Customer-Supplied Garments".
  // Each item's manifest is the rep-typed Quote_Items.Notes.
  items
    .filter(i => i.EmbellishmentType === 'customer-supplied'
      && typeof i.Notes === 'string' && i.Notes.trim())
    .forEach(i => notes.push({
      Type: NOTE_TYPES.RECEIVING,
      Note: `${i.ProductName || i.StyleNumber} (qty ${i.Quantity}):\n${i.Notes.trim()}`
    }));
  if (session.Carrier) {
...
module.exports = {
  ...
  buildPurchasingNote,
  buildShippingNote,
  buildUnbilledNote, // shared by the SCP + DTF transformers (2026-09-07): an unrouted item is never silent
  isCapProduct,
