...
function transformQuoteToOrder(session, items, options = {}) {
  ...
  return {
    ...
    // Tax: Let OnSite calculate from line item sts_EnableTax flags (matches Python InkSoft pattern)
    // ManageOrders integration auto-creates unwanted tax LINE ITEMS if TaxTotal > 0
    // Actual tax info preserved in Notes On Order for manual review/entry
    TaxTotal: 0,
    TaxPartNumber: '',
    TaxPartDescription: '',
...
function buildProductLines(item, garmentDesignId, capDesignId) {
  ...
  if (sizeEntries.length > 0) {
    ...
    for (const [size, qty] of sizeEntries) {
      ...
      lines.push({
        PartNumber: item.StyleNumber || '', // BASE only — OnSite Size Translation Table adds suffix
...
function buildDesigns(session, items) {
  ...
  // Garment: only link when we have a known numeric design ID
  if (garmentDesignNumber && /^\d+$/.test(garmentDesignNumber)) {
    designs.push({ id_Design: parseInt(garmentDesignNumber) });
  }
  // If empty/non-numeric — skip. Sales rep assigns design manually in ShopWorks.

  // Cap: only link when we have a known numeric design ID
...
function buildNotes(session, items, skippedFeeNotes = [], taxInfo = {}) {
  ...
  if (!session.GarmentDesignNumber && !session.CapDesignNumber) {
