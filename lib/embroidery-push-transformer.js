...
function buildNotes(session, items, skippedFeeNotes = [], taxInfo = {}) {
  ...
  if (taxAccountCode) {
    notes.push({ Type: NOTE_TYPES.ORDER, Note: `Tax Account: ${taxAccountCode} (${taxAccountDesc})` });
  } else if (taxAccountDesc) {
    notes.push({ Type: NOTE_TYPES.ORDER, Note: taxAccountDesc }); // "MANUAL REVIEW: ..." message
  }

  // Expected tax amount for reference
  const taxAmount = parseFloat(session.TaxAmount) || 0;
  if (taxAmount > 0) {
    notes.push({ Type: NOTE_TYPES.ORDER, Note: `Expected Tax: $${taxAmount.toFixed(2)}` });
  }

  // Unrecognized fee item descriptions
  if (skippedFeeNotes.length > 0) {
    notes.push({ Type: NOTE_TYPES.ORDER, Note: `Order notes: ${skippedFeeNotes.join(', ')}` });
  }
  ...
  if (importNotes) {
    notes.push({ Type: NOTE_TYPES.ORDER, Note: `Import Notes: ${importNotes}` });
  }
