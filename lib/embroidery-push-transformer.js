...
function transformQuoteToOrder(session, items, options = {}) {
  ...
  const linesOE = buildLinesOE(session, items);
  ...
  // Build notes
  const notes = buildNotes(session, items);
  ...
  return {
    ...
    id_Customer: EMB_ONSITE_DEFAULTS.id_Customer,
...
function buildLinesOE(session, items) {
  ...
  for (const item of items) {
    ...
    if (type === 'embroidery') {
      ...
      // DECG/DECC — single line
      lines.push(buildServiceLine(item, null, null));
    } else if (type === 'fee') {
      // Fee items — skip order-level ones
      const pn = (item.StyleNumber || '').toUpperCase();
...
function buildServiceLine(item, garmentDesignId, capDesignId) {
  // AL items link to the design they belong to
  let designId = '';
 ...
 */
function buildNotes(session, items) {
  const notes = [];

  // Notes On Order — order notes + PO + quote link
