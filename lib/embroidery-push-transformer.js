...
function transformQuoteToOrder(session, items, options = {}) {
  ...
  const { accountCode: taxAccountCode, description: taxAccountDesc } = getTaxAccount(taxRate, shipState);
  ...
  return {
    ...
    _effective_tax_rate: taxRate,
    cur_Shipping: shippingTotal,
    TotalDiscounts: discountTotal,

    // Tax array — mechanism OnSite actually processes for tax
    // Pattern from live InkSoft orders (Skyline Properties #52521, WA Hospitality #52520)
    Tax: taxAmount > 0 ? [{
      AccountName: '',
      TaxAmount: taxAmount,
      TaxRate: taxRate,
...
function buildLinesOE(session, items) {
  ...
  const seq = extractSequence(session.QuoteID);
 ...
 *
 * When no design number is available, falls back to the detailed block with
 * ExtDesignID, DesignName, Locations, etc. for ShopWorks to create a new design.
 */
function buildDesigns(session, items) {
  ...
  // If design number is known, use simple InkSoft pattern: {id_Design: N}
  // ShopWorks links to existing design — no duplicate creation
  if (garmentDesignNumber && /^\d+$/.test(garmentDesignNumber)) {
    designs.push({ id_Design: parseInt(garmentDesignNumber) });
  } else {
    // No known design — build detailed block for ShopWorks to create new design
    const { logoSpecs, firstProductColor } = extractLogoInfo(items);
  ...
  // Cap design (if caps exist)
  if (capDesignNumber || hasCapItems(items)) {
    if (capDesignNumber && /^\d+$/.test(capDesignNumber)) {
      designs.push({ id_Design: parseInt(capDesignNumber) });
    } else {
      const { logoSpecs, firstProductColor } = extractLogoInfo(items);
...
/**
 * Extract LogoSpecs and first product color from items (shared by buildDesigns paths)
 */
...
function buildNotes(session, items, skippedFeeNotes = [], taxInfo = {}) {
  ...
  // Notes On Order — order notes + PO + quote link + tax account + skipped fee descriptions
  ...
  // Tax account info (mirrors InkSoft's "Tax Account: 2200 (WA Sales Tax 10.1%)" note)
  const { taxAccountCode, taxAccountDesc } = taxInfo;
  if (taxAccountCode) {
    orderNoteParts.push(`Tax Account: ${taxAccountCode} (${taxAccountDesc})`);
  } else if (taxAccountDesc) {
    orderNoteParts.push(taxAccountDesc); // Contains "MANUAL REVIEW: ..." message
  }

  // Expected tax amount for reference (ShopWorks calculates actual tax from account + flags)
  const taxAmount = parseFloat(session.TaxAmount) || 0;
  if (taxAmount > 0) {
    orderNoteParts.push(`Expected Tax: $${taxAmount.toFixed(2)}`);
  }
...
function buildPurchasingNote(items) {
  ...
  for (const item of productItems) {
    ...
    if (sizeEntries.length > 0) {
      for (const [size, qty] of sizeEntries) {
        if (!qty || qty <= 0) continue;
        parts.push(`${style} - ${color} - ${size} x${qty} - $${price.toFixed(2)}`);
      }
    } else {
      // OSFA or no size breakdown
      const qty = parseInt(item.Quantity) || 1;
