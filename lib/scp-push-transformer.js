...
const { isKnownFeeCode, buildSalesTaxNote, buildAccountingTaxNote, isWholesaleSession, resolveTaxAccount } = require('../config/manageorders-emb-config');

const { getPartNumber } = require('../config/size-suffix-config');
const { buildUnbilledNote } = require('./embroidery-push-transformer');
...
function transformQuoteToOrder(session, items, options = {}) {
  ...
  const designBlock = `G-${String(session.QuoteID || '0')}`;
  ...
  const { accountCode: taxAccountCode, description: taxAccountDesc } =
    resolveTaxAccount({ taxRate, shipState, isWholesale: isWholesaleSession(session) });
  ...
  return {
    ...
    // Routes to the OnSite "Manage Orders" integration (APISource filter = "ManageOrders").
    // The integration imports ONLY orders whose APISource matches exactly, so stamp it on
    // every push or it's silently skipped. Erik 2026-06-04: "ManageOrders" on everything.
...
function buildLinesOE(session, items) {
  ...
  const seq = String(session.QuoteID || '0');
  ...
  for (const item of items) {
    ...
    if (type === 'screenprint') {
      ...
      for (const [size, qty] of Object.entries(sizes)) {
        ...
        // Base PN ONLY — ShopWorks's Size Translation Table appends the per-size
        // modifier (_2X, _3XL, _OSFA, …) from the Size field on ingest. Pre-suffixing
        // here double-stamps it (PC54_2X_2X). Verified vs the live SW table 2026-06-02.
        const partNumber = item.StyleNumber;
    ...
    // Catch-all (2026-09-07, ported from the EMB transformer's 2026-06-10 audit): an
    // EmbellishmentType this loop does not recognise used to fall off the end of the
    // iteration with no line, no note and no warning — the pushed order was simply short
    // by that item. The contract calculators already write 'customer-supplied' items; any
    // such row now lands in the order notes as an explicit UNBILLED call-to-action.
    if (type) {
...
function buildFeeLines(session, designBlock) {
  ...
  // 1. Screen setup — SPSU "Screen Print Set Up Charge" (Erik's official part,
  //    2026-06-27), $30/screen. Prefer the authoritative values the SCP service
  //    writes into Notes; fall back to deriving from the color counts. Per-screen
  //    price is derived from the quoted setupFeeTotal (which already used the Caspio
  //    SPSU sell price) so a Caspio price change flows through; $30 is the last
  //    resort. (SPRESET is a real part but not builder-driven; if a saved quote_item
  //    carries it, buildLinesOE passes it through as a known fee code.)
  ...
  if (screens > 0) {
    const perScreen = setupTotal > 0 ? Math.round((setupTotal / screens) * 100) / 100 : 30;
    lines.push(buildLineOE({
      partNumber: 'SPSU',
      description: `Screen Print Set Up Charge (${screens} screen${screens === 1 ? '' : 's'} × $${perScreen.toFixed(2)})`,
      color: '', size: '', qty: screens, price: perScreen, designBlock,
    }));
  }
  ...
  // 6. Vellum print ($10/print) — Erik's official setup part (2026-06-27). Qty +
  //    authoritative total come from Notes (written by the SCP quote-service);
  //    per-unit is derived from the quoted total so a Caspio price change flows.
  const vellumQty = parseInt(notes.vellumQty, 10) || 0;
  if (vellumQty > 0) {
    const vellumTotal = parseFloat(notes.vellumTotal) || 0;
    const vellumPrice = vellumTotal > 0 ? Math.round((vellumTotal / vellumQty) * 100) / 100 : 10;
    lines.push(buildLineOE({
      partNumber: 'Vellum', description: 'Vellum Print',
      color: '', size: '', qty: vellumQty, price: vellumPrice, designBlock,
    }));
  }

  // 7. Color change ($15 each) — Erik's official setup part (2026-06-27). PartNumber
  //    is 'Color Chg' verbatim (matches the ShopWorks part + Caspio Service_Codes).
  const colorChangeQty = parseInt(notes.colorChangeQty, 10) || 0;
  if (colorChangeQty > 0) {
    const ccTotal = parseFloat(notes.colorChangeTotal) || 0;
    const ccPrice = ccTotal > 0 ? Math.round((ccTotal / colorChangeQty) * 100) / 100 : 15;
    lines.push(buildLineOE({
      partNumber: 'Color Chg', description: 'Color Change',
      color: '', size: '', qty: colorChangeQty, price: ccPrice, designBlock,
    }));
  }
 ...
 * location (front, back, AND each sleeve).
 */
function deriveScreenCountFromNotes(notes) {
  const fc = parseInt(notes.frontColors, 10) || 0;
  const bc = parseInt(notes.backColors, 10) || 0;
  const sleeves = Array.isArray(notes.sleeveColorsList)
    ? notes.sleeveColorsList.map((c) => parseInt(c, 10) || 0).filter((c) => c > 0)
    : [];
  const sleeveScreens = sleeves.reduce((a, c) => a + c, 0);
  let screens = fc + bc + sleeveScreens;
  if (notes.isDarkGarment) screens += (fc > 0 ? 1 : 0) + (bc > 0 ? 1 : 0) + sleeves.length;
  return screens;
}
...
function formatScreenPrintSpec(rawNotes) {
  ...
  const lc = parseInt(n.leftSleeveColors, 10) || 0;
  const rc = parseInt(n.rightSleeveColors, 10) || 0;
  if (n.frontLocation || fc) parts.push(`Front: ${n.frontLocation || '?'} (${fc}c)`);
  if (n.backLocation || bc) parts.push(`Back: ${n.backLocation} (${bc}c)`);
  if (lc) parts.push(`L Sleeve (${lc}c)`);
  if (rc) parts.push(`R Sleeve (${rc}c)`);
...
/**
 * Build a detailed PRODUCTION-floor note (locations, colors per location,
 * screens to burn incl. underbase, garment darkness, safety stripes) — routed
 * to "Notes To Production" so the press floor sees it on the work order rather
 * than only the abbreviated Notes-On-Order summary. Added 2026-06-02 (Erik).
 */
function formatScreenPrintProductionNote(rawNotes) {
  if (!rawNotes) return '';
  let n = {};
  try {
    n = typeof rawNotes === 'string' ? JSON.parse(rawNotes || '{}') : (rawNotes || {});
  ...
  const lc = parseInt(n.leftSleeveColors, 10) || 0;
  const rc = parseInt(n.rightSleeveColors, 10) || 0;
  if (!fc && !bc && !lc && !rc && !n.frontLocation && !n.backLocation) return '';
  const ubF = (n.isDarkGarment && fc > 0) ? 1 : 0;
  const ubB = (n.isDarkGarment && bc > 0) ? 1 : 0;
  const ubL = (n.isDarkGarment && lc > 0) ? 1 : 0;
  const ubR = (n.isDarkGarment && rc > 0) ? 1 : 0;
  const underbase = ubF + ubB + ubL + ubR;
  const sleeveScreens = lc + rc;
  const totalScreens = fc + bc + sleeveScreens + underbase;
  const lines = ['SCREEN PRINT — PRODUCTION SPECS'];
  if (n.frontLocation || fc) lines.push(`Front: ${n.frontLocation || '(location?)'} — ${fc} color${fc === 1 ? '' : 's'}`);
  if (n.backLocation || bc) lines.push(`Back: ${n.backLocation || '(location?)'} — ${bc} color${bc === 1 ? '' : 's'}`);
  if (lc) lines.push(`Left Sleeve — ${lc} color${lc === 1 ? '' : 's'}`);
  if (rc) lines.push(`Right Sleeve — ${rc} color${rc === 1 ? '' : 's'}`);
  lines.push(`Screens to burn: ${totalScreens} (${fc} front + ${bc} back${sleeveScreens ? ` + ${sleeveScreens} sleeve` : ''}${underbase ? ` + ${underbase} white underbase` : ''})`);
  lines.push(n.isDarkGarment ? 'Garment: DARK — white underbase required' : 'Garment: light — no underbase');
  lines.push(`Safety stripes: ${n.hasSafetyStripes ? 'YES' : 'no'}`);
  return lines.join('\n');
}
...
function buildDesigns(session, items) {
  ...
  if (newName && hostedFiles.length > 0) {
    // Ink-color counts for the SW operator, from the saved Notes. Per-location
    // when the placement is clearly front/back (2026-07-10 Swagger field audit —
    // the old front+back SUM overstated colors on every location); ambiguous
    // placements keep the sum. Dark garments flag the white-underbase flash.
    const frontColors = parseInt(notes.frontColors, 10) || 0;
    const backColors = parseInt(notes.backColors, 10) || 0;
    const totalColors = frontColors + backColors;
    const isDark = notes.isDarkGarment === true || notes.isDarkGarment === 'true';
    const seq = String(session.QuoteID || '0');
    const locations = hostedFiles.map((f, i) => {
      const isBack = /back/i.test(String(f.placement || ''));
      const locColors = (isBack ? backColors : frontColors) || totalColors;
      return {
        Location: f.placement || 'Front',
        ImageURL: swImageUrl(f.hostedUrl), // P2: ≤2MB JPEG variant
        DesignCode: `SCP-${i + 1}`,
        TotalColors: locColors ? String(locColors) : '',
        TotalFlashes: isDark ? '1' : '',
        Notes: [f.fileName || '', isDark ? 'Underbase (dark garment)' : ''].filter(Boolean).join(' · '),
      };
    });
    return [{
...
function buildNotes(session, items, skippedFeeNotes, { taxAccountCode, taxAccountDesc, taxRate = 0, taxAmount = 0, shipping = 0 }) {
  const notes = [];

  // 1. Tax block — DTG/order-form style, one fact per row (shared helper) so all
  //    four push paths read identically: subtotal / rate / amount / total /
  //    GL account / "apply manually" (or EXEMPT / out-of-state). Erik 2026-06-02.
  //    [2026-06-11] now passes shipping — TaxAmount is computed on the shipping-
  //    inclusive base (quote-service 2026-06-08), so omitting it printed a note
  //    whose figures didn't reconcile and understated the order total.
  buildSalesTaxNote({
    subtotal: session.SubtotalAmount, shipping, taxRate, taxAmount,
  ...
  // [2026-06-11] EMB-parity accountant cross-check (Erik 2026-06-07, for Bradley).
  ...
  // 2b. Detailed PRODUCTION-floor instructions (colors per location, screens to
  //     burn incl. underbase, safety stripes) → Notes To Production so the press
  //     floor sees it on the production work order (Erik 2026-06-02).
  const prodSpec = formatScreenPrintProductionNote(session.Notes);
  if (prodSpec) {
    notes.push({ Type: NOTE_TYPES.PRODUCTION, Note: prodSpec });
  }
