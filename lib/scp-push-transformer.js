...
function formatScreenPrintProductionNote(rawNotes) {
  ...
  const fc = parseInt(n.frontColors, 10) || 0;
  const bc = parseInt(n.backColors, 10) || 0;
  if (!fc && !bc && !n.frontLocation && !n.backLocation) return '';
  const ubF = (n.isDarkGarment && fc > 0) ? 1 : 0;
  const ubB = (n.isDarkGarment && bc > 0) ? 1 : 0;
  const underbase = ubF + ubB;
  const totalScreens = fc + bc + underbase;
  ...
  lines.push(`Screens to burn: ${totalScreens} (${fc} front + ${bc} back${underbase ? ` + ${underbase} white underbase` : ''})`);
