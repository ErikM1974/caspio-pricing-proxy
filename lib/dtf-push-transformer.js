...
const { buildSalesTaxNote, buildAccountingTaxNote, isWholesaleSession, resolveTaxAccount } = require('../config/manageorders-emb-config');
...
function transformQuoteToOrder(session, items, options = {}) {
  ...
  const extOrderId = generateDtfExtOrderID(session.QuoteID, isTest, getQuoteYear(session));
  ...
  return {
    ...
    // Notes — ManageOrders /onsite/order-push reads order-level notes under
    // `Notes` (proven by manageorders-push-client.js:241 + the EMB transformer).
    // The earlier `NotesOnOrders` key was silently dropped by the API, so the
    // tax account + NO-DESIGN-LINKED flag never reached SW. (Fixed 2026-06-01.)
    Notes: notes,
...
function buildLinesOE(session, items) {
  ...
  for (const item of items) {
    if (item.EmbellishmentType !== 'dtf') {
      // DTF quotes store only garment rows as items — but a row of any OTHER type (a
      // 'customer-supplied' item from the contract calculators, a stray fee) used to be
      // skipped with no trace, leaving the pushed order short. Surface it as an explicit
      // UNBILLED note instead (2026-09-07; same safety net the EMB transformer has had
      // since the 2026-06-10 audit).
      if (item.EmbellishmentType) skippedFeeNotes.push(buildUnbilledNote(item, `UNBILLED ITEM [${item.EmbellishmentType}]`));
      continue;
    }
    ...
    for (const [size, qty] of Object.entries(sizes)) {
      ...
      // here double-stamps it (29M_2X_2X). Verified vs the live SW table 2026-06-02.
...
function buildDesigns(session, items) {
  ...
  if (newName && hostedFiles.length > 0) {
    ...
    // Per-location transfer size from the saved pricing snapshot
    // (Notes.transferBreakdown[{location, locationName, size, sizeName}]) —
    // matched by placement so the SW design location shows which gang-sheet
    // size the quote priced (2026-07-10 Swagger field audit). No match → just
    // the filename, exactly as before.
    const breakdown = Array.isArray(notes.transferBreakdown) ? notes.transferBreakdown : [];
    const sizeFor = (placement) => {
      const p = String(placement || '').toLowerCase().replace(/[^a-z]/g, '');
      const hit = breakdown.find((b) => {
        const ln = String(b && (b.locationName || b.location) || '').toLowerCase().replace(/[^a-z]/g, '');
        return ln && p && (ln === p || ln.includes(p) || p.includes(ln));
      });
      return hit && hit.sizeName ? String(hit.sizeName) : '';
    };
    const locations = hostedFiles.map((f, i) => {
      const sizeName = sizeFor(f.placement);
      return {
        Location: f.placement || 'Left Chest',
        ImageURL: swImageUrl(f.hostedUrl), // P2: ≤2MB JPEG variant
        DesignCode: `DTF-${i + 1}`,
        Notes: [f.fileName || '', sizeName ? `${sizeName} transfer` : ''].filter(Boolean).join(' · '),
      };
    });
...
function buildNotes(session, items, skippedFeeNotes, { taxAccountCode, taxAccountDesc }) {
  ...
  const taxRate = toRateDecimal(session.TaxRate);
  const taxAmount = parseFloat(session.TaxAmount) || 0;
  // [2026-06-11] shipping was omitted here — the note's "Total with Tax" came up
  // short by the shipping fee and its Tax Amount didn't reconcile with its own
  // Subtotal (TaxAmount is computed on the shipping-INCLUSIVE base since
  // 2026-06-08), inviting a wrong manual tax entry. EMB has passed it since
  // 2026-06-07; DTF/SCP missed the parity propagation.
  const { shippingTotal: shippingForNote } = extractOrderLevelFees(session);
  buildSalesTaxNote({
  ...
  // same breakdown as a single Notes To Accounting entry.
  ...
  // 4b. Transfer location(s) → Notes To Production so the press floor knows
  //     where to heat-press the transfer (Erik 2026-06-02).
  const dtfLocs = [...new Set(
    (Array.isArray(parsedNotes.referenceArtwork) ? parsedNotes.referenceArtwork : [])
      .map(f => f && f.placement).filter(Boolean)
  )];
  if (dtfLocs.length > 0) {
    notes.push({
      Type: NOTE_TYPES.PRODUCTION,
      Note: `DTF TRANSFER — PRODUCTION SPECS\n` +
            `Location${dtfLocs.length === 1 ? '' : 's'}: ${dtfLocs.join(', ')}\n` +
            `Full-color transfer — heat press per artwork.`,
    });
  }
