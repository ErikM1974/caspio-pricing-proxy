...
function createFormDataFromBase64(base64Data, fileName) {
    ...
    // Create temp file.
    //
    // 🔴 SECURITY (fixed 2026-08-05): fileName is CALLER-SUPPLIED and used to be
    // interpolated raw — `path.join(tempDir, \`upload_${Date.now()}_${fileName}\`)`.
    // path.join normalises, and the `upload_<ts>_` prefix is just one more segment
    // for `..` to pop, so a name like `../../../app/server.js` resolved straight out
    // of the temp dir:
    //     path.join('/tmp', 'upload_123_../../../app/server.js') === '/app/server.js'
    // POST /api/manageorders/orders/create reaches this with no auth (guardReadsOnly
    // gates GET only), and the write lands BEFORE the Caspio token call, so this was
    // an unauthenticated arbitrary-file write on the dyno — and the error path then
    // unlink()s the same traversed path, i.e. arbitrary delete.
    //
    // basename() strips every directory component; the character filter kills the
    // leftovers (a bare `..`, NULs, separators on either platform); the resolve()
    // assertion is belt-and-braces so any future edit that reintroduces a path
    // fails loudly here instead of silently writing outside tempDir.
    const tempDir = os.tmpdir();
    const safeName = path.basename(String(fileName == null ? '' : fileName))
        .replace(/[^A-Za-z0-9._-]/g, '_')
        .replace(/^\.+/, '_')
        .slice(0, 100) || 'upload';
    const tempFilePath = path.join(tempDir, `upload_${Date.now()}_${safeName}`);
    if (!path.resolve(tempFilePath).startsWith(path.resolve(tempDir) + path.sep)) {
        throw new Error('Refusing to write upload outside the temp directory');
    }
    ...
    // The CASPIO-facing filename deliberately keeps the caller's original string —
    // only the on-disk path had to be neutralised, so uploads still land in Caspio
    // under the name the caller intended.
    // (form-data requires a string here — a non-string name used to throw before
    // the upload was even attempted, so fall back to the sanitised one.)
    const formData = new FormData();
    formData.append('Files', fs.createReadStream(tempFilePath), {
        filename: typeof fileName === 'string' && fileName ? fileName : safeName,
...
module.exports = {
    ...
    validateFile,
    extractMimeType,
    // Exported for tests/jest/upload-filename-traversal.test.js — the temp path
    // this builds is reachable from an UNAUTHENTICATED request, so its
    // sanitisation is jest-locked rather than left to review.
    createFormDataFromBase64
};
