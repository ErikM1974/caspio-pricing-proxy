...
const INDUSTRIES = [
    // ===== Public Safety (highly specific, do first) =====
    {
        ...
        patterns: [
            { kw: /\bfire\s+(department|district|rescue|prevention|protection|station|engine|chief)\b/i, confidence: 'high' },
            { kw: /\b(police|sheriff|deputy|marshal|trooper|patrol|swat|k-?9)\b/i, confidence: 'high' },
            { kw: /\b(EMT|EMS|paramedic|ambulance|rescue\s+squad)\b/i, confidence: 'high' },
    ...
    // ===== Education =====
    {
        industry: 'Education',
        patterns: [
            { kw: /\b(elementary|middle|high)\s+school\b/i, confidence: 'high' },
    ...
    // ===== Manufacturing =====
    {
        ...
        patterns: [
            { kw: /\b(manufacturing|manufacturer|mfg|industries|industrial)\b/i, confidence: 'high' },
    ...
    // ===== Retail =====
    {
        industry: 'Retail',
        patterns: [
            { kw: /\b(boutique|store|shop|retail|mercantile|outfitters?|outlet)\b/i, confidence: 'medium' },
            { kw: /\b(automotive\s+(group|sales|dealership)|auto\s+(sales|dealer)|chevrolet|ford|toyota|honda)\b/i, confidence: 'high' },
            { kw: /\b(supply\s+(co|company|store|inc|llc)|hardware|farm\s+supply|feed\s+store)\b/i, confidence: 'medium' },
        ],
    },
    ...
    // ===== Government / Municipal =====
    {
        ...
        patterns: [
            ...
            { kw: /\b(municipal|public\s+works|department\s+of\s+|state\s+of\s+)/i, confidence: 'high' },
            { kw: /\b(parks?\s+(and|&)\s+(rec|recreation)|water\s+(district|department|utility)|utility\s+district)\b/i, confidence: 'high' },
            { kw: /\b(transit\s+(authority|agency)|public\s+transit|metro\s+transit)\b/i, confidence: 'high' },
        ],
    },
