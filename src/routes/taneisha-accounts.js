...
// GET /api/taneisha-accounts/reverse-gap-report - Find orders by OTHER reps for customers IN Taneisha's CRM
// This shows orders that inflate her CRM total but weren't processed by her
router.get('/taneisha-accounts/reverse-gap-report', async (req, res) => {
    try {
        console.log('Generating Taneisha reverse gap report (orders by OTHER reps for HER customers)...');
        const { fetchOrders, getDateDaysAgo } = require('../utils/manageorders');
        ...
        // Find orders for Taneisha's customers BUT written by OTHER reps
        const reverseGapCustomers = new Map();
        allOrders.forEach(order => {
            // Must be a customer IN Taneisha's list
            if (taneishaCustomerIds.has(order.id_Customer)) {
                // BUT the order must be written by a DIFFERENT rep
                if (order.CustomerServiceRep !== 'Taneisha Clark') {
        ...
        res.json({
            ...
            explanation: 'Orders by OTHER reps for customers in Taneisha\'s CRM. These inflate her accounts total but weren\'t processed by her.',
            customers: reverseGapList
        });
    } catch (error) {
        console.error('Error generating Taneisha reverse gap report:', error.message);
...
router.put('/taneisha-accounts/:id/crm', express.json(), async (req, res) => {
    ...
    try {
        ...
        if (affected === null) {
            ...
            console.warn(`[taneisha-accounts] CRM update for ${custId}: unrecognised Caspio response shape`);
        }
...
router.post('/taneisha-accounts/sync-sales', express.json(), async (req, res) => {
    try {
        ...
        orders.forEach(order => {
            ...
            // Only process orders for Taneisha's currently-assigned customers.
            // accountMap is built from Taneisha_Accounts, which sync-ownership
            // populates from Sales_Reps_2026 (the source-of-truth customer →
            // current account manager mapping). So this filter alone gives us
            // the territory view: every order Taneisha's customers placed counts
            ...
            // === 'Taneisha Clark' that excluded orders Taneisha's customers
            // placed but someone else invoiced. That was the order-time snapshot
            // field, which doesn't survive customer reassignment, and made this
...
// POST /api/taneisha-accounts/sync-ownership - Sync customer list from Sales_Reps_2026 (ShopWorks source of truth)
// This keeps Taneisha's CRM table in sync with ShopWorks customer assignments
router.post('/taneisha-accounts/sync-ownership', express.json(), async (req, res) => {
    try {
        const REP_NAME = 'Taneisha Clark';
        console.log('Starting Taneisha accounts ownership sync from Sales_Reps_2026...');
        ...
        // 4. Add new customers (customers now assigned to Taneisha in ShopWorks)
            ...
            try {
                await axios({
                    ...
                    data: {
                        ID_Customer: customer.ID_Customer,
                        CompanyName: customer.CompanyName,
                        Account_Tier: customer.Account_Tier || "BRONZE '26-TANEISHA",
                        Is_Active: 1
                    },
                    timeout: 10000
                });
        ...
        // 5. Remove customers no longer assigned to Taneisha (reassigned in ShopWorks)
        ...
        console.error('Error syncing Taneisha ownership:', error.message);
