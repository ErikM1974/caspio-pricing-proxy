...
// GET /api/nika-accounts/gap-report - Find orders by Nika for customers NOT in her CRM list
// This shows the detailed orders causing the gap between "processed" and "accounts" totals
router.get('/nika-accounts/gap-report', async (req, res) => {
    try {
        console.log('Generating Nika gap report...');
        const { fetchOrders, getDateDaysAgo } = require('../utils/manageorders');
        ...
        // Filter to Nika's orders only
        const nikaOrders = allOrders.filter(o => o.CustomerServiceRep === 'Nika Lao');
        console.log(`Found ${nikaOrders.length} orders by Nika`);
        ...
        nikaOrders.forEach(order => {
            if (!nikaCustomerIds.has(order.id_Customer)) {
                // This customer is NOT in Nika's CRM - their orders cause the gap
        ...
        console.error('Error generating Nika gap report:', error.message);
