// Nika Daily Sales Archive routes - For YTD tracking beyond ManageOrders 60-day limit
// Table: Nika_Daily_Sales_By_Account (SalesDate, CustomerID, CustomerName, Revenue, OrderCount, ArchivedAt)

const express = require('express');
const router = express.Router();
const { fetchAllCaspioPages, makeCaspioRequest } = require('../utils/caspio');

const TABLE_NAME = 'Nika_Daily_Sales_By_Account';
...
router.get('/daily-sales-by-account', async (req, res) => {
  ...
  console.log(`GET /api/nika/daily-sales-by-account requested with start=${start}, end=${end}`);

  // Validate required parameters
  if (!start || !end) {
    return res.status(400).json({
      error: 'Both start and end date parameters are required',
      example: '/api/nika/daily-sales-by-account?start=2026-01-01&end=2026-01-31'
    });
  }
  ...
  try {
    ...
    console.log(`Nika daily sales by account: ${records.length} records across ${days.length} days, ${summaryCustomers.length} customers`);
    ...
    console.error('Error fetching Nika daily sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to fetch Nika daily sales by account',
...
router.get('/daily-sales-by-account/ytd', async (req, res) => {
  ...
  console.log(`GET /api/nika/daily-sales-by-account/ytd requested for year=${year}`);

  try {
    const records = await fetchAllCaspioPages(`/tables/${TABLE_NAME}/records`, {
      'q.where': `SalesDate>='${yearStart}' AND SalesDate<='${yearEnd}'`,
      'q.orderBy': 'SalesDate DESC',
      'q.limit': 5000 // ~400 customers x ~365 days / archiving frequency
    });
    ...
    console.log(`Nika YTD by account ${year}: $${totalRevenue.toFixed(2)} from ${records.length} records, ${customers.length} customers`);
    ...
    console.error('Error fetching Nika YTD sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to fetch Nika YTD sales by account',
      details: error.message
    });
  }
});
...
router.post('/daily-sales-by-account', async (req, res) => {
  ...
  console.log(`POST /api/nika/daily-sales-by-account requested with date=${date}, customers=${customers?.length || 0}`);
  ...
  try {
    ...
    console.log(`Nika daily sales by account archived for ${date}: ${customersArchived} customers (${results.created} created, ${results.updated} updated)`);
    ...
    console.error('Error archiving Nika daily sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to archive Nika daily sales by account',
      details: error.message
    });
  }
});
...
router.post('/daily-sales-by-account/bulk', async (req, res) => {
  const days = req.body;
  console.log(`POST /api/nika/daily-sales-by-account/bulk requested with ${days?.length || 0} days`);
