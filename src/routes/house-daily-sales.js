...
const TABLE_NAME = 'House_Daily_Sales_By_Account';
...
router.get('/daily-sales-by-account', async (req, res) => {
  ...
  console.log(`GET /api/house/daily-sales-by-account requested with start=${start}, end=${end}`);

  // Validate required parameters
  if (!start || !end) {
    return res.status(400).json({
      error: 'Both start and end date parameters are required',
      example: '/api/house/daily-sales-by-account?start=2026-01-01&end=2026-01-31'
    });
  }
  ...
  try {
    ...
    console.log(`House daily sales by account: ${records.length} records across ${days.length} days, ${summaryCustomers.length} customers`);
    ...
    console.error('Error fetching House daily sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to fetch House daily sales by account',
...
router.get('/daily-sales-by-account/ytd', async (req, res) => {
  ...
  console.log(`GET /api/house/daily-sales-by-account/ytd requested for year=${year}`);
  ...
  try {
    ...
    console.log(`House YTD by account ${year}: $${totalRevenue.toFixed(2)} from ${records.length} records, ${customers.length} customers`);
    ...
    console.error('Error fetching House YTD sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to fetch House YTD sales by account',
      details: error.message
    });
  }
});
...
router.post('/daily-sales-by-account', async (req, res) => {
  ...
  console.log(`POST /api/house/daily-sales-by-account requested with date=${date}, customers=${customers?.length || 0}`);
  ...
  try {
    ...
    console.log(`House daily sales by account archived for ${date}: ${customersArchived} customers (${results.created} created, ${results.updated} updated)`);
    ...
    console.error('Error archiving House daily sales by account:', error.message);
    ...
    res.status(500).json({
      error: 'Failed to archive House daily sales by account',
      details: error.message
    });
  }
});
...
router.post('/daily-sales-by-account/bulk', async (req, res) => {
  const days = req.body;
  console.log(`POST /api/house/daily-sales-by-account/bulk requested with ${days?.length || 0} days`);
