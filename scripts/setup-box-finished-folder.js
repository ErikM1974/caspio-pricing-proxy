// Setup: create/find the "Finished Product Photos" Box folder under BOX_ART_FOLDER_ID
// (writable by the CCG service account) and print its ID. Mirrors setup-box-archive-folder.js.
// Run once on Heroku (has the Box env vars):  heroku run node scripts/setup-box-finished-folder.js
// Then set:  heroku config:set BOX_FINISHED_PHOTO_FOLDER_ID=<printed id>
...
const NAME = 'Finished Product Photos';
...
async function main() {
  ...
  try {
    ...
    console.log('CREATED  BOX_FINISHED_PHOTO_FOLDER_ID =', resp.data.id);
  } catch (e) {
    if (e.response && e.response.status === 409) {
      ...
      if (found) { console.log('EXISTS  BOX_FINISHED_PHOTO_FOLDER_ID =', found.id); return; }
    }
